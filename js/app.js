...
const flashcardsModeButton =
    document.querySelector("#flashcardsModeButton");

const memoryModeButton =
    document.querySelector("#memoryModeButton");

const flashcardsSection =
    document.querySelector("#flashcardsSection");
...
const flashcardLabel =
    document.querySelector("#flashcardLabel");

const flashcardText =
    document.querySelector("#flashcardText");

//Selects Flash card progress counters
const flashcardProgress =
    document.querySelector("#flashcardProgress");

const gotItCount =
    document.querySelector("#gotItCount");

// Selects Flashcard control buttons
const flipButton =
    document.querySelector("#flipButton");

const againButton =
    document.querySelector("#againButton");

const gotItButton =
    document.querySelector("#gotItButton");

// Selects the flascard compeltion area
const flashcardComplete =
    document.querySelector("#flashcardComplete");

const flashcardScoreMessage =
    document.querySelector("#flashcardScoreMessage");

const restartFlashcardsButton =
    document.querySelector("#restartFlashcardsButton");
...
const moveCount =
    document.querySelector("#moveCount");

const matchCount =
    document.querySelector("#matchCount");
...
const restartMemoryButton =
    document.querySelector("#restartMemoryButton");

const memoryWin =
    document.querySelector("#memoryWin");

const memoryWinMessage =
    document.querySelector("#memoryWinMessage");
...
function switchMode(mode) {
    ...
    flashcardsSection.classList.toggle(
        "hidden",
        !showFlashcards
    );

    memorySection.classList.toggle(
        "hidden",
        showFlashcards
    );

    // updates the active appearence of the mode buttons
    flashcardsModeButton.classList.toggle(
        "active",
        showFlashcards
    );

    memoryModeButton.classList.toggle(
        "active",
        !showFlashcards
    );

    // Upades accessibilty information
    flashcardsModeButton.setAttribute(
        "aria-pressed",
        String(showFlashcards)
    );

    memoryModeButton.setAttribute(
        "aria-pressed",
        String(!showFlashcards)
    );
    ...
    if (showFlashcards) {
        ...
        const firstMemoryCard =
            memoryGrid.querySelector(".memory-card");
...
function showFlashcardResults() {
    ...
    document
        .querySelector(".flashcard-controls")
...
function startMemoryGame() {
    ...
    matchCount.textContent =
        `Matches: 0 / ${memoryPairs.length}`;

    memoryStatus.textContent =
        "Select two cards to look for a match.";
...
function flipMemoryCard(cardElement) {
    ...
    const cardIndex =
        Number(cardElement.dataset.index);

    const cardData =
        memoryDeck[cardIndex];
...
function checkForMatch() {
    ...
    const firstIndex =
        Number(firstSelectedCard.dataset.index);

    const secondIndex =
        Number(secondSelectedCard.dataset.index);

    // Gets Data for both cards
    const firstCardData =
        memoryDeck[firstIndex];

    const secondCardData =
        memoryDeck[secondIndex];

    // Cards match when their PairID values are the same
    if (
        firstCardData.pairId ===
        secondCardData.pairId
    ) {
...
function handleMatch(matchedCardData) {
    ...
    firstSelectedCard.classList.add(
        "matched"
    );

    secondSelectedCard.classList.add(
        "matched"
    );
    ...
    firstSelectedCard.setAttribute(
        "aria-label",
        `Matched card: ${matchedCardData.pairId}`
    );

    secondSelectedCard.setAttribute(
        "aria-label",
        `Matched card: ${matchedCardData.pairId}`
    );
    ...
    matchCount.textContent =
        `Matches: ${matches} / ${memoryPairs.length}`;
...
function handleMismatch() {
    ...
    memoryStatus.textContent =
        "No match. The cards will turn back over.";

    //waits before turing the cards facedown
    window.setTimeout(() => {
        firstSelectedCard.classList.remove(
            "flipped"
        );

        secondSelectedCard.classList.remove(
            "flipped"
        );
...
function showMemoryWin() {
    ...
    memoryWinMessage.textContent =
        `You matched all ${memoryPairs.length} pairs in ${moves} moves.`;
...
flashcardsModeButton.addEventListener(
    "click",
    () => {
...
memoryModeButton.addEventListener(
    "click",
    () => {
        switchMode("memory");
    }
...
restartFlashcardsButton.addEventListener(
    "click",
    restartFlashcards
);

// restarts and reshuffles the memory game
restartMemoryButton.addEventListener(
    "click",
    startMemoryGame
);
...
memoryGrid.addEventListener(
    ...
    (event) => {
        ...
        if (
            !selectedCard ||
            !memoryGrid.contains(selectedCard)
        ) {
            return;
        }
